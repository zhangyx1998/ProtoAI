...
#define MAX_DESC_BUF_SIZE                                                      \
  32 // Max length of string descriptor (can be extended, USB supports lengths
     // up to 255 bytes)
...
typedef struct {
  ...
  const uint8_t *fs_cfg; /*!< Pointer to Full-speed configuration descriptor,
                            always present. */
  const uint8_t *hs_cfg; /*!< Pointer to High-speed configuration descriptor,
                            NULL when device Full-speed only. */
#if (TUD_OPT_HIGH_SPEED)
  const tusb_desc_device_qualifier_t
      *qualifier; /*!< Pointer to Qualifier descriptor. */
  uint8_t
      *other_speed; /*!< Pointer for other speed configuration descriptor. */
#endif              // TUD_OPT_HIGH_SPEED
  const char *str[USB_STRING_DESCRIPTOR_ARRAY_SIZE]; /*!< Pointer to array of
                                                        UTF-8 strings. */
...
//   return (TUSB_SPEED_HIGH == tud_speed_get()) ? s_desc_cfg.hs_cfg
//                                               : s_desc_cfg.fs_cfg;
// }
...
uint8_t const *tud_descriptor_other_speed_configuration_cb(uint8_t index) {
  ...
  memcpy(s_desc_cfg.other_speed, other_speed,
         ((tusb_desc_configuration_t *)other_speed)->wTotalLength);

  ((tusb_desc_configuration_t *)s_desc_cfg.other_speed)->bDescriptorType =
...
// {
//     (void) langid; // Unused, this driver supports only one language in
//     string descriptors assert(s_desc_cfg.str); uint8_t chr_count; static
//     uint16_t _desc_str[MAX_DESC_BUF_SIZE];
...
//             ESP_LOGW(TAG, "String index (%u) is out of bounds, check your
//             string descriptor", index); return NULL;
//         }

//         if (s_desc_cfg.str[index] == NULL) {
//             ESP_LOGW(TAG, "String index (%u) points to NULL, check your
//             string descriptor", index); return NULL;
//         }

//         const char *str = s_desc_cfg.str[index];
//         chr_count = strnlen(str, MAX_DESC_BUF_SIZE - 1); // Buffer len -
//         header
...
//     descriptor type (TUSB_DESC_STRING) _desc_str[0] = (TUSB_DESC_STRING << 8
//     ) | (2 * chr_count + 2);
...
esp_err_t tinyusb_descriptors_check(tinyusb_port_t port,
                                    const tinyusb_desc_config_t *config) {
  ESP_RETURN_ON_FALSE(config, ESP_ERR_INVALID_ARG, TAG,
                      "Descriptors config can't be NULL");
  ESP_RETURN_ON_FALSE(config->string_count <= USB_STRING_DESCRIPTOR_ARRAY_SIZE,
                      ESP_ERR_NOT_SUPPORTED, TAG,
                      "String descriptors exceed limit");

#if (SOC_USB_OTG_PERIPH_NUM > 1)
  if (port == TINYUSB_PORT_HIGH_SPEED_0) {
#if !TUD_OPT_HIGH_SPEED
    ESP_RETURN_ON_FALSE(false, ESP_ERR_INVALID_ARG, TAG,
                        "Device has only Full-speed port");
#endif
  }
#endif
...
esp_err_t tinyusb_descriptors_set(tinyusb_port_t port,
                                  const tinyusb_desc_config_t *config) {
  esp_err_t ret;
  ...
  if (config->full_speed_config == NULL) {
    ...
    ESP_LOGW(TAG,
             "No Full-speed configuration descriptor provided, using default.");
    s_desc_cfg.fs_cfg = descriptor_fs_cfg_default;
#else
    // Default configuration descriptor must be provided via config structure
    ESP_GOTO_ON_FALSE(
        config->full_speed_config, ESP_ERR_INVALID_ARG, fail, TAG,
        "Full-speed configuration descriptor must be provided for this device");
#endif
  } else {
  ...
  if (port == TINYUSB_PORT_HIGH_SPEED_0) {
    ...
    if (config->high_speed_config == NULL) {
      ...
      ESP_LOGW(
          TAG,
          "No High-speed configuration descriptor provided, using default.");
      s_desc_cfg.hs_cfg = descriptor_hs_cfg_default;
#else
      // High-speed configuration descriptor must be provided via config
      // structure
      ESP_GOTO_ON_FALSE(config->high_speed_config, ESP_ERR_INVALID_ARG, fail,
                        TAG,
                        "High-speed configuration descriptor must be provided "
                        "for this device");
#endif
    } else {
    ...
    // Other Speed Descriptor buffer allocation, will be used for other speed
    // configuration descriptor request
    uint16_t other_speed_buf_size =
        MAX(((tusb_desc_configuration_t *)s_desc_cfg.fs_cfg)->wTotalLength,
            ((tusb_desc_configuration_t *)s_desc_cfg.hs_cfg)->wTotalLength);
    s_desc_cfg.other_speed = calloc(1, other_speed_buf_size);
    ESP_GOTO_ON_FALSE(s_desc_cfg.other_speed, ESP_ERR_NO_MEM, fail, TAG,
                      "Other speed memory allocation error");
#endif // TUD_OPT_HIGH_SPEED
  } else {
  ...
  ESP_GOTO_ON_FALSE(s_desc_cfg.str_count <= USB_STRING_DESCRIPTOR_ARRAY_SIZE,
                    ESP_ERR_NOT_SUPPORTED, fail, TAG,
                    "String descriptors exceed limit");
  memcpy(s_desc_cfg.str, pstr_desc,
         s_desc_cfg.str_count * sizeof(pstr_desc[0]));
  ...
  ESP_LOGI(TAG,
           ...
           s_desc_cfg.dev->idVendor, s_desc_cfg.dev->idProduct,
           s_desc_cfg.dev->bcdDevice, s_desc_cfg.dev->iManufacturer,
           s_desc_cfg.dev->iProduct, s_desc_cfg.dev->iSerialNumber,
